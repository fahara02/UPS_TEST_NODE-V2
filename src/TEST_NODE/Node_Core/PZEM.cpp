...
{
	localSWserial =
		new SoftwareSerial(receivePin, transmitPin); // We will need to clean up in destructor
...
PZEM::PZEM(Stream& port, uint8_t addr) :
{
	init(&port, true, addr);
}
#endif
...
{
	_addr = (addr >= MIN_SLAVE_ADDR && addr <= MAX_SLAVE_ADDR) ? addr : PZEM_DEFAULT_ADDR;
	_serial = port;
	_isSoft = isSoft;
	_lastRead = -1;
	_isConnected = false;
...
uint16_t PZEM::readVoltage()
{
	return readSingleReg(Registers::VOLTAGE, 0x01);
}
...
uint32_t PZEM::readPower()
{
	return readSingleReg(Registers::POWER_L, 0x02);
}

uint32_t PZEM::readEnergy()
{
	return readSingleReg(Registers::ENERGY_L, 0x02);
}
...
uint16_t PZEM::readPowerFactor()
{
	return readSingleReg(Registers::PF, 0x01);
}
...
bool PZEM::setAddress(uint8_t newaddr)
{
	...
	if(!sendCmd(FunctionCode::WRITE_SINGLE_REG, Registers::SLAVE_ADDR, newaddr, true))
		return false;
				   ...
				   uint16_t slave_addr)
{
	...
	if((slave_addr == 0xFFFF) || (slave_addr < MIN_SLAVE_ADDR) || (slave_addr > MAX_SLAVE_ADDR))
	{
		slave_addr = _addr;
	}
...
ReceivedResult PZEM::receive(uint8_t* resp, uint16_t len)
{
	...
	return {checkCRC(resp, len) ? ReceiveStatus::SUCCESS : ReceiveStatus::CRC_ERROR, index};
}
...
uint32_t PZEM::readSingleReg(uint16_t rAddr, uint8_t rLength, RegisterType regType)
{
	uint8_t respBuffer[SINGLE_REG_RESPONSE];
	FunctionCode cmd = (regType == RegisterType::HOLDING_REG) ? FunctionCode::READ_HOLDING_REG :
...
bool PZEM::checkCRC(const uint8_t* buf, uint16_t len)
{
	...
	return (buf[len - 2] == (crc & 0xFF)) && (buf[len - 1] == (crc >> 8)); // Check CRC
}
...
{
	return (static_cast<uint16_t>(response[startIndex]) << 8) | response[startIndex + 1];
}
...
{
	uint8_t buffer[] = {0x00, static_cast<uint8_t>(FunctionCode::RESET), 0x00, 0x00};
	uint8_t reply[5];
	buffer[0] = _addr;
...
void PZEM::search()
{
	...
	{
		if(sendCmd(FunctionCode::READ_INPUT_REG, Registers::VOLTAGE, 0x01, true, i))
		{
			receive(respBuffer, SINGLE_REG_RESPONSE); // Check response

			if(respBuffer[0] == i)
			{ // If response matches address
