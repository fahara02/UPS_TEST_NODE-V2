...
UPSTest<T, U, testype>::UPSTest() :
	_data(), _cfgSpec(TesterSetup->specSetup()), _cfgTest(TesterSetup->testSetup()),
	_cfgTask(TesterSetup->taskSetup()), _cfgTaskParam(TesterSetup->paramSetup()),
	_cfgHardware(TesterSetup->hardwareSetup()), _initialized(false), _runTest(false),
	_testinProgress(false), _dataCaptureRunning(false), _dataCaptureOk(false), _currentTest(0),
	_testDuration(TesterSetup->testSetup().testDuration_ms)
{
	...
	TesterSetup->registerTestCallback([this](bool test_updated, SetupTest setting) {
		this->onTestUpdate(test_updated, setting);
	});
	TesterSetup->registerTaskCallback([this](bool task_updated, SetupTask setting) {
	...
	TesterSetup->registerHardwareCallback([this](bool hardware_updated, SetupHardware setting) {
...
{
	const uint16_t maxVARating = _cfgSpec.Rating_va; // Assuming maxVA rating is in TestSettings
