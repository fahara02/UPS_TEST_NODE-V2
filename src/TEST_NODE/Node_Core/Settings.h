...
enum class TestMode { AUTO = 0, MANUAL = 1 };
...
struct SetupUPSTest {
  ...
  SetupUPSTest(SetupSpec sp = SetupSpec(), SetupTest ts = SetupTest(),
               SetupTask tk = SetupTask(),
               SetupTaskParams ps = SetupTaskParams(),
               SetupHardware hw = SetupHardware(),
               SetupNetwork comm = SetupNetwork(),
               SetupModbus mb = SetupModbus(), SetupReport rp = SetupReport(),
               unsigned long lsu = 0)
      : spec(sp),
        testSetting(ts),
        taskSetting(tk),
