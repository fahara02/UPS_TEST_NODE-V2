...
private:
  ...
  const std::array<Transition, 20> regular_transitions
  ...
  const std::array<Transition, 12> special_case_transitions
      = {{{State::TEST_IN_PROGRESS, Event::TEST_TIME_END,
  ...
  const std::array<Transition, 32> transition_table = [this] {
    std::array<Transition, 32> result
        = {};  // Initialize an empty array of 32 transitions

    // Copy the regular transitions
    std::copy(regular_transitions.begin(), regular_transitions.end(),
              result.begin());
